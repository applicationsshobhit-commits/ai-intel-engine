...
SOURCES = [
    ...
    ("google_news:salesforce", partial(fetch_query, "Salesforce stock OR earnings OR investment", "google_news/salesforce", WEEK)),
    ("google_news:crm-competition", partial(fetch_query, "Salesforce competitors OR Microsoft Dynamics OR HubSpot", "google_news/crm-competition", WEEK)),
    ("google_news:ai-innovation", partial(fetch_query, "AI innovation enterprise software", "google_news/ai-innovation", WEEK)),
]
...
def main():
    ...
    for name, fetch in SOURCES:
        try:
            items = [i for i in fetch() if is_this_week(i)]
        ...
        total_new += new_count
        print(f"[{name}] fetched {len(items)} (this week), {new_count} new")
